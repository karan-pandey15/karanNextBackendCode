...
const partner_csv_route = express();

partner_csv_route.use(bodyParser.urlencoded({ extended: false }));
partner_csv_route.use(bodyParser.json());
  ...
  try {
    ...
    let fileStream = csv
      ...
      .on("end", async () => {
        ...
        await PFormalData.insertMany(formDataArray);

        if (pendingFormDataArray.length > 0) {
          await PPendingFormData.insertMany(pendingFormDataArray);
        }

        if (disbursedDataArray.length > 0) {
          await PDisbursedData.insertMany(disbursedDataArray);
        }

        if (rejectedDataArray.length > 0) {
          await PRejectedData.insertMany(rejectedDataArray);
        }
...
export default partner_csv_route;
